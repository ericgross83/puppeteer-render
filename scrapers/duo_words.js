...
async function scrapeDuolingoWords(headless = true) {
    ...
    const browser = await puppeteer.launch({
        ...
        args: [
            ...
            '--disable-blink-features=AutomationControlled' // Versteckt Bot-Eigenschaften besser
        ]
    });

    const timeout = 60000; // Erhöht auf 60s für Railway-Stabilität
    ...
    try {
        ...
        // --- NEU: "USE ANOTHER ACCOUNT" HANDLING ---
        try {
            // Wir prüfen, ob der Button aus deinem Screenshot existiert
            const clickedAnother = await page.evaluate(() => {
                const elements = [...document.querySelectorAll('button, a, span')];
                const target = elements.find(el => el.innerText.includes('USE ANOTHER ACCOUNT'));
                if (target) {
                    target.click();
                    return true;
                }
            ...
            if (clickedAnother) {
                console.log('✅ "USE ANOTHER ACCOUNT" geklickt.');
                // Kurz warten, damit die Seite umschalten kann
                await new Promise(r => setTimeout(r, 2000)); 
            }
        } catch (e) {
            console.log('Kein "USE ANOTHER ACCOUNT" Button gefunden, fahre normal fort.');
        }

        // 2. Login Modal öffnen
        await page.waitForSelector('button[data-test="have-account"]', { visible: true });
        await page.click('button[data-test="have-account"]');

        // 3. Login Daten eingeben
        await page.waitForSelector('input[data-test="email-input"]', { visible: true });
        ...
        try {
            ...
            console.log('📸 Screenshot "duolingo_error.png" erstellt.');
        } catch (screenshotError) {
            console.error('Screenshot fehlgeschlagen:', screenshotError.message);
        }
        throw error;
    } finally {
        await browser.close();
